...
<%
	...
	synchronized (session) {
		...
		if (hwGrades == null) {
			...
			final String oneStudentId = null;
			hwGrades = new GradeSet(course.getId(), assgts, oneStudentId, !TUTORIALS);
		}
...
<body style="background-color:#f6f7ed; margin:0px; overflow:auto;" <%=
	...
	<tr><td class="boldtext big" style="padding-left:10px; 
		...
		<% if (numAttempts > 1) { %>
			...
			<input type="hidden" name="isInstructorOrTA" 
					value="<%= isInstructorOrTA %>" />
	...
	<tr><td colspan="2" 
		...
		<% } // if comment %>
		<% if (!isText) { %>
			<tr><td class="regtext" style="padding-left:10px;" id="code">
				<a onclick="showSource();"><%= user.translate("Source code") %></a>.
			</td></tr>
		<% } // if question type %>
	</table>
</body>
