...
<%	
	...
	final String coinstructor = request.getParameter("coinstructor");

	String message = "";
	try {
		if (userSess instanceof AdminSession) {
			((AdminSession) userSess).removeCoinstructor(crsId, 
					coinstructor);
		} else if (userSess instanceof InstructorSession) {
			((InstructorSession) userSess).removeCoinstructor(crsId, 
					coinstructor);
...
<body class="light" style="background-color:white;">
	...
	<% } else { %>
		<tr><td>
			Coinstructor removed successfully.  
		</td></tr>
		<tr><td style="padding-top:10px;">
