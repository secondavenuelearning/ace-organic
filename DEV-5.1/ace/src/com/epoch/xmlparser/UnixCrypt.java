...
/** Utilities for encrypting passwords.  From org.w3c.util. */
public final class UnixCrypt 
{
	...
	private static int fourBytesToInt(byte[] b, int offset) {
		...
		value  =  byteToUnsigned(b[localOffset++]);
		value |= (byteToUnsigned(b[localOffset++]) <<  8);
		value |= (byteToUnsigned(b[localOffset++]) << 16);
		value |= (byteToUnsigned(b[localOffset++]) << 24);
		return value;
	}

	private static void intToFourBytes(int iValue, byte[] b, int offset) {
		int localOffset = offset;
		b[localOffset++] = (byte)((iValue)		  & 0xff);
		b[localOffset++] = (byte)((iValue >>> 8 ) & 0xff);
		b[localOffset++] = (byte)((iValue >>> 16) & 0xff);
		b[localOffset++] = (byte)((iValue >>> 24) & 0xff);
	}
...
schedule);
	...
	*/
	public static String crypt(String salt, String original)
	{
	  ...
	  while(localSalt.length() < 2)
		 localSalt += "A";

	  final StringBuilder buffer = Utils.getBuilder("			 ");

	  final char charZero = localSalt.charAt(0);
	  final char charOne  = localSalt.charAt(1);
	...
	*/
	public static String crypt(String original) {
		...
		final String salt = Utils.toString(
