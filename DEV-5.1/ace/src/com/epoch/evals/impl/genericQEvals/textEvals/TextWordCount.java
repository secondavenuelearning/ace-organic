...
/** If the number of words in the response {is, is not} {=, &lt;, &gt;} <i>n</i> ... */ 
public class TextWordCount extends CompareNums implements EvalInterface {
	 ...
	 * evaluator
	 */
	public TextWordCount(String data) throws ParameterException {
		...
		if (splitData.length < 2 || getOper() == -1) {
			throw new ParameterException("TextWordCount ERROR: "
					+ "unknown input data '" + data + "'. ");
		}
	} // TextWordCount(String)
	...
	public String toEnglish() {
		return Utils.toString("If the number of words in the response is",
				OPER_ENGLISH[FEWER][getOper()], authNum);
	} // toEnglish()

	/** Determines whether the response contains the indicated number of words.  
	...
	public OneEvalResult isResponseMatching(Response response, 
		...
		final int wordCt = resp.split(" ").length;
		debugPrint("TextWordCount.isResponseMatching: number of words in ",
				resp, " is ", wordCt, ".");
		evalResult.isSatisfied = compare(wordCt, authNum);
		return evalResult;
	 ...
	 */
	public String getMatchCode() 			{ return EVAL_CODES[TEXT_WORDS]; } 
	/** Gets the number of words against which to compare. 
	 * @return	number of words against which to compare
	 */
	public int getAuthNum() 				{ return authNum; } 
	/** Sets the number of words against which to compare. 
	 * @param	num	number of words against which to compare
	 */
