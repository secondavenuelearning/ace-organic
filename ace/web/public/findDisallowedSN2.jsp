...
<%
	...
	MechRuleFunctions mechAnalyzer = null;
	int numAtoms = 0;
	if (substrate != null) {
		final Mechanism mechanism = new Mechanism(substrate);
		mechAnalyzer = new MechRuleFunctions(mechanism);
...
<head>
<title>Disallowed SN2 reaction finder</title>
...
<!-- self submit form -->
<form name="tester" action="findDisallowedSN2.jsp" method="post">
<input type="hidden" name="substrate" value=""/>
<p class="boldtext big" style="text-align:center;">
ACE Disallowed S<sub>N</sub>2 reaction finder
</p>
...
<% if (numAtoms > 1) { %>
	...
	<tr><td class="regtext" >
		...
		<%  
		try { mechAnalyzer.containsSN2(HYB_MATTERS); %>
			does not contain an S<sub>N</sub>2 reaction at 
			an sp- or sp<sup>2</sup>-hybridized atom.
		<% } catch (Exception e) { %>
			contains an S<sub>N</sub>2 reaction at an sp- 
			or sp<sup>2</sup>-hybridized atom.
		<% } %>
	<p>Try again, if you like!
<% } else { %>
	<tr><td class="regtext">
		Draw a mechanistic step in the left-hand box
		with electron-flow arrows, 
		and press the button below to see whether this mechanism 
		contains an S<sub>N</sub>2 reaction at an sp- or 
...
<td style="text-align:center;" colspan="2">
	<br/>
	<input type="button" value=" Find Disallowed SN2 " onclick="getDisallowedSN2()"/>
	<br/>
	<br/>
</td>
</tr>
